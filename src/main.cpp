    ...
    bool enableThreadIdCollection)
{
    ...
    return std::make_shared<folly::IOThreadPoolExecutor>(
        std::min(folly::hardware_concurrency(), 4u),
...
namespace folly
{
  extern std::atomic<folly::EventBaseManager *> globalManager;
}
  ...
  if(1){
        ...
        folly::globalManager.exchange(&ebm);
        auto *evb= ebm.getEventBase();
            //auto *evb = folly::EventBaseManager::get()->getEventBase();
        // use the same EventBase for the main thread
        //folly::EventBaseManager::get()->setEventBase(evb, true);
        // if (evb)
        // {
        //   ebm.setEventBase(evb, true);
        // }
        auto *ev_backend = evb->getBackend();
        printf("smth\n");
#else
